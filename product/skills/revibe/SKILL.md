...
At every stage boundary, require the stage to write `.revibe/handoffs/<stage>.md`, set `awaiting_review`, and use the shared interactive user-question protocol to review all meaningful findings, recommendations, uncertainties, and choices with the user. The final question must be the open-ended addition check, and the stage must incorporate its response before setting `complete`. Read the handoff back with the state and require matching `output_revision` values. Unanswered items remain pending. A recommendation is never approval.
