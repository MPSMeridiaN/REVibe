...
The controller (the active stage owner) is the only actor that asks the user. Before the first question, enumerate every meaningful unresolved choice for the stage, order dependent questions, and keep asking until each is answered, explicitly deferred, or blocked. If a structured question capability exists and is permitted in the current mode, use it for every user-facing question in the review; never switch to plain conversational questions merely for convenience. Ask one decision at a time by default. Batch only genuinely independent questions when the capability supports batching; otherwise make repeated calls and use each answer before asking a dependent question. An asynchronous question is pending until its actual answer arrives; do not infer an answer from a successful tool call, timeout, or preselected option.

Every question must carry a review context packet before the question itself:

```text
subject: the named feature, decision, risk, or proposed change being reviewed
claim: what REVibe currently believes about it
evidence: source refs, artifact path, worker assignment/result IDs, and confidence
impact: what accepting, rejecting, or deferring changes downstream
...
Before sending a question, run a strict question-quality linter gate: verify that all five prerequisite context components are populated:
1. Subject (exact named feature or boundary)
2. Plain-language meaning (practical boundary/behavior)
3. Origin reference (file/line, artifact, test, or worker packet)
4. Observed evidence and confidence
5. Downstream impact

If any of items 1–5 is missing or ambiguous, asking the decision question is strictly blocked; the controller must collect missing evidence or ask a prerequisite clarification first. Reject any question where nouns such as “features,” “options,” or “the change” are not defined in the cited artifact, where the user cannot identify the subject from the question alone, or that combines multiple dependent decisions. Store the rendered context packet in the handoff Trace so a later controller can reproduce exactly what the user was shown.
...
At stage start, discover the harness's agent and question capabilities and relevant tools. For agents, inspect the exposed catalog and schemas for a capability that can create or dispatch a bounded subagent and return its final result; names such as `spawn_agent`, `delegate`, `create_task`, or `subagent` are only search hints. Confirm the capability by schema before calling it. Use the user's model preferences where supported; do not require a vendor-specific model in the portable product. If one or more permitted subagent capabilities are present, call them for every substantive stage assignment before doing that assignment in the controller. A substantive stage cannot advance to `awaiting_review` without recording at least one dispatched `worker_assignment_id` in its handoff Trace. A capability discovery result without a dispatch call is a failure of orchestration. If no permitted subagent capability exists, record the catalog evidence and perform the worker/controller roles sequentially; do not claim independent review. If a capability exists but dispatch fails, preserve the error, retry only when safe, and keep the stage `in_progress` or `blocked` rather than silently doing the delegated work alone.
...
Workers return the evidence packet above plus their run ID, assignment ID, consumed baseline/revision, result (`done`, `partial`, or `blocked`), changed files or artifact references, checks actually run and their outcomes, and unresolved items. Reject mismatched run IDs before consuming results. The controller must enforce strict packet schema compliance: if a worker returns unstructured text omitting required keys (`scope:`, `question:`, `finding:`, `evidence:`, `confidence:`, `gaps:`), the controller must reject the packet and require structured re-submission before incorporating findings. A worker's `done` means its assignment is finished, never that the stage is accepted. Workers do not write canonical state or stage handoffs, ask the user, approve decisions, or dispatch a downstream stage.
...
4. Replace the handoff and state as one logical transaction, preserving `.bak` copies where supported. The state replacement is the commit point; verify by reading the canonical state and its referenced handoff back and checking that both output revisions equal the canonical `revision`. When Python execution is permitted by the harness, execute the bundled verification script (e.g. `python <skill-path>/scripts/check_state.py . [--run <run-id>]`) to guarantee zero schema drift or broken cross-references before dispatching the next stage.
...
## Context bounding and fresh session resumption

When an assignment spans multiple intensive stages or the host conversation approaches context window limits (>60%), the controller should commit the current stage checkpoint, record `run.status: paused`, and provide `/revibe resume <run-id>` as the resumption command. Because `.revibe/<run-id>/` is completely self-contained and durable, resuming in a fresh session eliminates prompt degradation and guarantees 100% adherence to all coordination and review rules.
