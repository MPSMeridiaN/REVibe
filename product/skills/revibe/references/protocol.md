...
At stage start, inspect the current harness and discover relevant agents, skills, MCPs, CLIs, language servers, build systems, test runners, browsers, debuggers, and analyzers. Use available capabilities when they improve evidence, but keep the core workflow portable. If a capability is absent, use a narrower single-agent or manual path and record the limitation as a constraint, question, or low-confidence gap.

Parallelize only independent scopes, such as separate features or read-only architecture lanes. Serialize work when it depends on a prior finding, changes shared files, performs a migration, or could make another worker's conclusion stale. Give each worker a precise question and return packet. An orchestrator merges packets, resolves contradictions, and writes the one authoritative state and handoff.
